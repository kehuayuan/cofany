$(function(){
		...
		onSelect : function(title){
			if(title=="常用语"){
				$("#oftenUseLanPg").treegrid({
					url : rootPath+"/talks/loadTalksLanguageModelData",
					rownumbers: true,
					animate: true,
					pagination : true,
					idField: 'id',
				...
				$("#oftenUseLinkPg").treegrid({
					url : rootPath+"/talks/loadTalksLinkModelData",
					rownumbers: true,
					animate: true,
					pagination : true,
					idField: 'id',
				...
				$("#attachmentTable").datagrid({
					url : rootPath+"/talks/loadTalksAttachmentData",
					rownumbers: true,
					animate: true,
					pagination : true,
					singleSelect: true,
				...
				$("#attachmentTypeTree").tree({
					url : rootPath+"/talks/loadTalksAttachmentTypeData",
					method : "post",
					animate : true,
	...
	/*话术分类数据*/
	$("#protypeId").tree({
		url : rootPath+"/talks/talksTeckniqueType",
		method: 'post',
		animate: true,
		onContextMenu: function(e,node){
			e.preventDefault();
	...
	$("#protypeData").datagrid({
		url : rootPath+"/talks/talksTecknique",
		method : "post",
		rownumbers : true,
		pagination : true,
		pageSize : 10,
...
function deleteProblem(){
	var obj = $("#protypeData").datagrid("getSelected");
	if(obj){
		$.messager.confirm('删除话术', '您确定要删除吗?', function(r){
			if (r){
				$.ajax({
					url : rootPath+"/talks/deleteTalksTecknique",
					type : "post",
					data : {ids:obj.ids},
					dataType : "json",
...
function submitProTypeForm(){
	...
	if(validateFlag && nameFlag){
		$.ajax({
			url : rootPath+"/talks/submitTalksTeckniqueType",
...
function deleteProType(){
	var treeObj = $("#protypeId").tree('getSelected');
	$.messager.confirm('删除话术分类', '您确定要删除吗?', function(r){
		if (r){
			$.ajax({
				url : rootPath+"/talks/deleteTalksTeckniqueType",
				type : "post",
				data : {ids:treeObj.id},
				dataType : "json",
				success : function(data){
...
function submitProblem(){
	$.ajax({
		url : rootPath+"/talks/submitTalksTecknique",
...
function submitOftenUseLanForm(){
	if($.trim($("#oftenUseLanName").textbox("getValue"))!=""){
		$.ajax({
			url : rootPath+"/talks/sumbitTalksLanguage",
...
function deleteOftenUseLan(){
	...
	$.messager.confirm('删除常用语类别', '您确定要删除吗?', function(r){
		if (r){
			$.ajax({
				url : rootPath+"/talks/deleteTalksLanguage",
				type : "post",
				data : {ids:treeObj.id},
				dataType : "json",
				success : function(data){
...
function submitOftenUseLinkForm(){
	...
	if(name != "" && flag){
		$.ajax({
			url : rootPath+"/talks/sumbitTalksLink",
...
function deleteOftenUseLink(){
	...
	$.messager.confirm('删除常用链接', '您确定要删除吗?', function(r){
		if (r){
			$.ajax({
				url : rootPath+"/talks/deleteTalksLink",
				type : "post",
				data : {ids:treeObj.id},
				dataType : "json",
				success : function(data){
...
function submitAttachmentType(){
	...
	if(attachmentName!=""){
		$.ajax({
			url : rootPath+"/talks/sumbitTalksAttachmentType",
...
function deleteAttachmentType(){
	...
	$.messager.confirm('删除附件分类', '您确定要删除吗?', function(r){
		if (r){
			$.ajax({
				url : rootPath+"/talks/deleteTalksAttachmentType",
				type : "post",
				data : {ids:treeObj.id},
				dataType : "json",
				success : function(data){
...
function addAttachment(){
	$.ajaxFileUpload({
		url : rootPath+"/talks/uploadTalksAttachmentFile",
		secureuri : false,
		fileElementId : "file1",
		dataType : "jsonp",
...
function editAttachment(){
	...
	if(obj!=null){
		$.ajaxFileUpload({
			url : rootPath+"/talks/uploadTalksAttachmentFile?ids="+obj.ids,
			secureuri : false,
			fileElementId : "file2",
			dataType : "jsonp",
			success : function(data, status){
...
function deleteAttachment(){
	...
	if(obj!=null){
		$.messager.confirm('删除附件', '您确定要删除吗?', function(r){
			if (r){
				$.ajax({
					url : rootPath+"/talks/deleteTalksAttachment",
					type : "post",
					data : {ids:obj.ids},
					dataType : "json",
					success : function(data){
